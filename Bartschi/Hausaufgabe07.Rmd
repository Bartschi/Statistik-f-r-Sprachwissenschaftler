...
```{r}
for(s in c(3,4,5) ){durchfall <- pnorm(5,mean=mu,sd=s)
...
```{r}
for(s in c(3,4,5)){
  bestnoten <- pnorm(13, mean=mu, sd=s, lower.tail=FALSE)
  output <- paste ("Bei einer Standabweichung von",s, "bestehen", bestnoten*100,"%  sehr gut.")
  print(output)
}
```
Wie steht die Anzahl guter Noten in Beziehung zur Anzahl schlechter Noten?
Es gibt mehr Studenten, die durchfallen, als Studenten, die mit "sehr gut" bestehen.

Warum?
Weil wir beim 2. Teil nur die Studenten betrachten, die die Notenpunkte 13-15 betrachten und sagen, dass alles was unter 13 Punkten ist nicht beachtet wird.
...
Die Verteilung ist rechtsschief, weil der Gipfel sich auf der linken Seite befindet.
...
`r (pnorm(10, mean=mu, sd=3)-pnorm(0, mean=mu, sd=3)*n`Studenten bekommen weniger als 10 Notenpunkten.
