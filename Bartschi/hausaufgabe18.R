...
lmx1 <- lm(y~x1, data=linreg)
summary(lmx1)

lmx2 <- lm(y~x2, data= linreg)
summary(lmx2)
...
# Berechnen Sie die zwei Regressionsmodelle für y ~ x1 und y ~ x2
 ...
 lm.x1 <- lm(y~x1, data=pyreg)
summary(lm.x1)

lm.x2 <- lm(y~x2, data=pyreg)
summary(lm.x2)
...
# Glauben Sie jetzt, dass y im linearen Verhältnis zu x1 und x2 steht? Warum (nicht)?
 Ich glaube nicht, dass y im linearen Verhältnis zu x1 und x2 steht, da die Punkte zu weit verstreut sind.
...
Bei diesen Daten macht Pearson am meisten Sinn, da es nicht um Ränge geht.
...
Die Werte y, x1 und y, x2 korrelieren miteinander. Die Werte x1 und x2 hingegen korrelieren nicht miteinander.
# Welche Methode macht hier am meisten Sinn? Korrelationen oder Regression?
Korrelation ist hier am sinnvollsten.
