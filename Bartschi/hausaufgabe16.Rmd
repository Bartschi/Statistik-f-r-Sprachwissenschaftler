...
Gibt es einen Einfluss von Tiefe und Farbe oder Cut auf den Preis? Ist der Einfluss der Tiefe oder von der Farbe oder des Cuts auf den Preis höher? Inwieweit gibt es einen Zusammenspiel zwischen Tiefe und Farbe bzw. Tiefe und Cut auf den Preis?
...
ggplot(diamonds, aes(x=z, y=price, color=color))+ geom_point()
ggplot(diamonds, aes(x=z, y=price, color=cut))+ geom_point()
ggplot(diamonds, aes(x=z, y=price, color=color))+ geom_point()+facet_wrap(~color)
ggplot(diamonds, aes(x=z, y=price, color=cut))+ geom_point()+facet_wrap(~color)
```

Lineare Regression
Einfluss von Tiefe auf den Preis?
```{r}
lm1 <- lm(price~z* color, data=diamonds)
summary(lm1)

lm2 <- lm(price~z* cut, data=diamonds)
summary (lm2)

```

Einfluss von Farbe auf den Preis?

```{r}
diamonds$color1 <- as.numeric(diamonds$color)
summary(lm(price~color1, data= diamonds))
```

Einfluss von Cut auf den Preis?
```{r}
diamonds$cut1 <- as.numeric(diamonds$cut)
summary(lm(price~cut1, data= diamonds))
```
...
```{r}
ANOVA.Tiefe <-aov(price ~ z, data=diamonds)
ANOVA.Farbe <-aov(price ~ color, data = diamonds)
ANOVA.Cut <- aov(price ~ cut, data = diamonds)
summary(ANOVA.Tiefe)
summary(ANOVA.Farbe)
ANOVA.Tiefe
ANOVA.Farbe
...
Zusammenspiel von Tiefe/Farbe auf den Preis
```{r}
ANOVA.TF<- aov(price~z*color, data = diamonds)
summary(ANOVA.TF)
```

Zusammenspiel von Tiefe/Cut auf den Preis

```{r}
ANOVA.TC<- aov(price~z*cut, data = diamonds)
summary(ANOVA.TC)
```
